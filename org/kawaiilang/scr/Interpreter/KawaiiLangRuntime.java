...
public final class KawaiiLangRuntime {
    ...
    //The case number the interpreter is currently interpreting to put into the switch statement
    ...
    //Whether or not the code should be interpreted, null and true will be interpreted and false will not.
    //Null means there is no preference, true is used in if statements that are true, and false is used in if statements that are false
    ...
    //A collection of all variables and tokens currently in use, built-in or custom mapped by the varname
    ...
    public Object interpret() {
        ...
        /*heap.forEach((K, V) -> {
          if (!(V.getValue() instanceof BuiltInFunction)) { 
        ...
        if (classDeclaring != null) {	//Declaring insides of class
          if (classHelper == null) {
        	...
        	classHelper = null;
        	heap.put(new TokenV1(TokenV1.TT_CLASS, classDeclaring.className()), new Variable(TokenV1.C_CLASSVT, classDeclaring));
        	classDeclaring = null;
          } else {
          ...
          if (tokens.length == 1 && tokens[0].equals(TokenV1.C_ENDFUNCK)) {
            ...
            if (tokens.length == 1 && tokens[0].equals(TokenV1.C_ENDLOOPK)) {
                ...
                if (tokens.length > 0 && tokens[0].equals(TokenV1.C_FORK) && tokens[tokens.length - 1].equals(TokenV1.C_TIMESK)) {
          ...
          if (switches.peek().equals(Switch.BLANK_SWITCH)) {
        	...
        	if (tokens.length > 2 && tokens[0].equals(TokenV1.C_TILDE) && tokens[1].equals(TokenV1.C_EQUALS)) {
        	  ...
        	  if (o instanceof Double) {
        		...
        		return new InvalidParameterError(start, pos, "Naooo uwu numwer tw switch mwst be awn intewer ._.");
        	  }
        	  return null;
            ...
            if (tokens.length == 2 && tokens[0].getType().equals(TokenV1.TT_STR) && tokens[1].equals(TokenV1.C_INCLUDE)) {
              ...
              try {
            	...
            	Position start = pos.clone();
            	return new RunTimeError(start, pos, ie.getMessage());
              }
              fn = thisFn;
              return null;
              ...
              StringBuilder sb = new StringBuilder(fn.split("(\\/(?!.*\\/)|\\\\(?!.*\\\\))(?!.*(\\/(?!.*\\/)|\\\\(?!.*\\\\)))")[0]);
          	  sb.append('/');
          	  sb.append(className);
              ...
              overload = true;
            } else if (tokens.length > 6 && tokens[0].equals(TokenV1.C_OWOK) && tokens[1].equals(TokenV1.C_FUNCK) && tokens[2].getType().equals(TokenV1.TT_VARNAME) && tokens[3].equals(TokenV1.C_PARAM) && tokens[tokens.length - 3].equals(TokenV1.C_PARAM) && tokens[tokens.length - 2].equals(new TokenV1(TokenV1.TT_KEYWORD, "canGibU")) && (tokens[tokens.length - 1].getType().equals(TokenV1.TT_VARTYPE) || tokens[tokens.length - 1].getType().equals(TokenV1.TT_NOTHING))) {
              ...
              while (true) {
                ...
                if (currentToken.equals(TokenV1.C_PARAM) || lastToken.equals(TokenV1.C_PARAM)) {
                  ...
                  } else if (lastToken.getType().equals(TokenV1.TT_VARTYPE) && currentToken.getType().equals(TokenV1.TT_VARNAME)) {
                    String paramType = (String) lastToken.getValue();
                    String paramName = (String) currentToken.getValue();
                    param.put(paramName, paramType);
                    advance();
            ...
            } else if (tokens.length > 2 && tokens[0].equals(TokenV1.C_WHILEK) && tokens[1].equals(TokenV1.C_STARTIF) && tokens[tokens.length - 1].equals(TokenV1.C_ENDIF)) {  //While loops
              Token[] condition = Arrays.copyOfRange(tokens, 2, tokens.length - 1);
              loop = new DoWhen(this, condition);
              declaringLoop = true;
              loopIdx++;
            } else if (tokens.length > 0 && tokens[0].equals(TokenV1.C_FORK) && tokens[tokens.length - 1].equals(new TokenV1(TokenV1.TT_KEYWORD, "twimes"))) { //For loops
                ...
                if (doInterpret == null || doInterpret == true) {
                    ...
                    if (Arrays.asList(tokens).contains(TokenV1.C_ANDK) || Arrays.asList(tokens).contains(TokenV1.C_ORK) || Arrays.asList(tokens).contains(TokenV1.C_XORK)) {
                        ArrayList < Token > exA = new ArrayList < > ();
                        ArrayList < Token > exB = new ArrayList < > ();
                        boolean secondPart = false;
                        Token oper = null;
                        for (int i = 0; i < tokens.length; i++) {
                            if (tokens[i].getValue() != null && (tokens[i].getValue().equals("awnd") || tokens[i].getValue().equals("orw") || tokens[i].getValue().equals("xwr"))) {
                                if (!secondPart) {
                                    oper = tokens[i];
                                    secondPart = true;
                                } else {
                    ...
                    //==, >, <, >=, <= operations
                    if (Arrays.asList(tokens).contains(new TokenV1(TokenV1.TT_EQUALS)) || Arrays.asList(tokens).contains(TokenV1.C_LT) || Arrays.asList(tokens).contains(TokenV1.C_GT) || Arrays.asList(tokens).contains(TokenV1.C_LTE) || Arrays.asList(tokens).contains(TokenV1.C_GTE)) {
                        ArrayList < Token > exA = new ArrayList < > ();
                        ArrayList < Token > exB = new ArrayList < > ();
                        boolean secondPart = false;
                        Token oper = null;
                        for (int i = 0; i < tokens.length; i++) {
                            if (tokens[i].getType() == TokenV1.TT_EQUALS || tokens[i].getType() == TokenV1.TT_LT || tokens[i].getType() == TokenV1.TT_GT || tokens[i].getType() == TokenV1.TT_LTE || tokens[i].getType() == TokenV1.TT_GTE) {
                                if (!secondPart) {
                                    oper = tokens[i];
                                    secondPart = true;
                                } else {
                    ...
                    while (true) {  //Main interpretation loop
                        if (currentToken == null) {
                            ...
                            return currentToken;
                        } else if (currentToken.getType().equals(TokenV1.TT_INT) || currentToken.getType().equals(TokenV1.TT_FLOAT)) {
                          ...
                          if (pos.getIdx() + 1 < tokens.length && tokens[pos.getIdx() + 1].getType() == TokenV1.TT_ASSIGN && heap.containsKey(currentToken)) {
                                lastToken = currentToken;
                                advance();
                                Object value = new Runner(fn, this).interpret(Arrays.copyOfRange(tokens, pos.getIdx() + 1, tokens.length));
                                if (value instanceof org.kawaiilang.Error) {
                          ...
                          if (varData != null) {
                                ...
                                if (type.getValue().equals("Numwer")) {
                                  if (stored instanceof Integer) {
                                    ...
                                    Position start = pos.clone();
                                    return new IllegalTypeError(start, pos, new StringBuilder("Naooo uwu da twype of variable, ").append(type.getType()).append(", is nawt as decwared ._.").toString());
                                  }
                                    ...
                                    return new IllegalTypeError(start, pos, new StringBuilder("Naooo uwu da twype of variable, ").append(type.getType()).append(", is nawt as decwared ._.").toString());
                                      ...
                                      while (true) {
                                        ...
                                        if (currentToken == null) {
                                          ...
                                          //Checks if last variable is a function, if so, it would assume its a nested function.
                                          if (lastToken != null && lastToken.getType().equals(TokenV1.TT_VARNAME) && heap.get(lastToken).getType().getValue().equals("Fwnctwion")) {
                                        	nestedParamCounter++;
                                      ...
                                      if (retrievedFunc.getParameterTypes() == null) {
                                        ...
                                        if (inputs != null) {
                                          ...
                                          if (ovld != null) {
                                        	...
                                        	Position start = pos.clone();
                                        	return new InvalidParameterError(start, pos, "Naooo uwu awgwmwents r giwen wen de fwnctwion does nawt hwas awgwments ._.");
                                    	...
                                    	if (inputs != null) {
                                    	  try {
                                    		...
                                    		if (ovld != null) {
                                              ...
                                              if (o == null) {
                                          		Position start = pos.clone();
                                          		return new InvalidParameterError(start, pos, new StringBuilder("Naooo uwu nao too mwany awgwmwents r giwen fwr the fwnctwion ").append(retrievedFunc.getName()).append(" ._.").toString());
                                          	  }
                                        ...
                                        if (inputs == null || o instanceof InvalidParameterError) {
                                          ...
                                          if (ovld != null) {
                                        	...
                                        	if (o == null) {
                                        	  Position start = pos.clone();
                                        	  return new InvalidParameterError(start, pos, "Naooo uwu nao awgwmwents r giwen wen de fwnctwion hwas awgwments ._.");
                                        	}
                                        	return o;	//No overloaded function so just return the error
                                          }
                                        }
                                        ...
                                        if (currentToken.getType().equals(TokenV1.TT_RSQU)) {
                                          if (at.isEmpty()) {
                                            if (!at2.isEmpty()) {	//Slice at end
                                              ...
                                              if (i >= l.length()) {
                                            	Position start = pos.clone();
                                              	return new IndexOutOfBoundsError(start, pos, "uwu naooo inwex iws owt ofw bwownds ._.");
                                              }
                                              ...
                                              Position start = pos.clone();
                                              return new InvalidParameterError(start, pos, "uwu naooo u must spwecify an inwex between the sqware bwackets ._.");
                                            }
                                          } else if (at2.isEmpty()) {
                                  ...
                                  if (tokens.length > 1 && tokens[pos.getIdx() + 1].equals(TokenV1.C_DOK)) {
                                	...
                                	if (o instanceof OwObject) {
                                	  ...
                                	  if (owoo.className().equals(type.getValue()) || owoo.hasParent((String)type.getValue())) {
                                		return owoo.doActions(Arrays.copyOfRange(tokens, pos.getIdx() + 2, tokens.length));
                                	  } else {
                                		Position start = pos.clone();
                                		return new IllegalTypeError(start, pos, new StringBuilder("Naooo uwu da twype of variable, ").append(type.getValue()).append(", is nawt as decwared ._.").toString());
                                	  }
                                	} else {
                                	  Position start = pos.clone();
                                	  return new IllegalTypeError(start, pos, new StringBuilder("Naooo uwu da twype of variable, ").append(type.getValue()).append(", is nawt as decwared ._.").toString());
                            ...
                            lastToken = currentToken;
                            advance();
                        } else if (lastToken == null || currentToken.getType() == TokenV1.TT_ADD || currentToken.getType() == TokenV1.TT_MINUS || currentToken.getType() == TokenV1.TT_MUL || currentToken.getType() == TokenV1.TT_DIV || currentToken.getType() == TokenV1.TT_MOD) {
                            if (lastToken != null && (lastToken.getType() == TokenV1.TT_ADD || lastToken.getType() == TokenV1.TT_MINUS)) {
                            ...
                            } else if (currentToken.getType() == TokenV1.TT_ADD || currentToken.getType() == TokenV1.TT_MINUS || currentToken.getType() == TokenV1.TT_MUL || currentToken.getType() == TokenV1.TT_DIV || currentToken.getType() == TokenV1.TT_MOD) {
                                if (lastToken != null && (lastToken.getType() != TokenV1.TT_INT && lastToken.getType() != TokenV1.TT_FLOAT)) {
                                ...
                                lastToken = currentToken;
                                advance();
                            } else if (currentToken.getType().equals(TokenV1.TT_VARTYPE) || currentToken.getType().equals(TokenV1.TT_CLASS)) {
                            //Variable assignment
                              	  ...
                              	  if (currentToken.getType().equals(TokenV1.TT_CLASS)) {
                              		...
                              		if (currentToken != null && currentToken.equals(TokenV1.C_PARAM)) {
                              		  ...
                              		  if (totalParamIndicators % 2 != 0) {
                              			Position start = pos.clone();
                              			return new InvalidSyntaxError(start, pos, "Unclosed parameter indicator");
                              		  }
                              		  totalParamIndicators--;
                              		  ArrayList<Token> param2eval = new ArrayList<>();
                              		...
                              		Position start = pos.clone();
                              		return new InvalidSyntaxError(start, pos, new StringBuilder("Naooo uwu \"").append(lastToken.toString()).append("\" is not a proper cwass ._.").toString());
                              	  }
                            ...
                            if (currentToken.getType() == TokenV1.TT_ADD || currentToken.getType() == TokenV1.TT_MINUS || currentToken.getType() == TokenV1.TT_MUL || currentToken.getType() == TokenV1.TT_DIV || currentToken.getType() == TokenV1.TT_MOD) {
                                expr.append(currentToken.getType());
                                lastToken = currentToken;
                                advance();
    ...
    //Gets the proper overloaded function given the name of the function and the parameter types
    ...
    public void addFunction(Function f) {
      heap.put(new TokenV1(TokenV1.TT_VARNAME, f.getName()),  new Variable(new TokenV1(TokenV1.TT_VARNAME, "Fwnctwion"), f));
    }
    ...
    //but different variables for very specific contexts, like position and if statement declarations
